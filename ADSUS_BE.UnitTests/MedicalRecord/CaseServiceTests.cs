...
public class CaseServiceTests
{
    ...
    [Fact]
    public async Task GetForPatientAsync_OwnEndedCase_ReturnsPatientResponse()
    {
    ...
    public async Task GetForPatientAsync_OwnCaseConfirmedButNotYetEnd_ThrowsResourceNotFoundExceptionWithSameMessage()
    {
        ...
        // chuyển sang End) vẫn phải bị giấu, kể cả với chính patient sở hữu ca đó và kể cả
        // khi gọi trực tiếp bằng ID (đảo ngược hành vi cũ chấp nhận cả Confirmed lẫn End).
        var patientUser = MedicalRecordTestData.MakePatientUser();
        var profile = MedicalRecordTestData.MakePatientProfile(patientUser);
        var confirmedNotEndedCase = MedicalRecordTestData.MakeCase(profile, status: CaseStatus.Confirmed);
    ...
    [Fact]
    public async Task ListMineAsync_AlwaysPassesEndStatusOnlyRegardlessOfCaller()
    {
        // Arrange — GB-05: server ép cứng {End}, không có tham số nào từ client tới được đây
        // để đổi giá trị này. Quyết định 14/08/2026 (sau khi trao đổi lại): CHỈ End mới coi là
        // "đã hoàn tất lượt khám" — ca mới Confirmed (chưa kê đơn) không hiện trong danh sách
        // của chính patient (đảo ngược quyết định {Confirmed, End} chốt cùng ngày trước đó).
        var patientUser = MedicalRecordTestData.MakePatientUser();
        var profile = MedicalRecordTestData.MakePatientProfile(patientUser);
        _profiles.Setup(r => r.GetByUserIdAsync(patientUser.UserId, It.IsAny<CancellationToken>()))
                 .ReturnsAsync(profile);
    ...
    [Fact]
    public async Task ConfirmAsync_CaseAlreadyConfirmed_ThrowsBusinessExceptionWithoutSaving()
    {
        // Arrange — P2/GB-01: CONFIRMED là trạng thái cuối, chốt lần hai bị từ chối kể cả
        // bởi chính bác sĩ phụ trách và kể cả gửi lại đúng nội dung cũ.
        var doctor = MedicalRecordTestData.MakeDoctor();
    ...
    [Fact]
    public async Task ConfirmAsync_CaseNotFound_ThrowsResourceNotFoundException()
    {
        // Arrange
        _cases.Setup(r => r.GetForUpdateAsync(It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
              .ReturnsAsync((Case?)null);
    ...
    [Fact]
    public async Task SaveConclusionAsync_ValidRequestByResponsibleDoctor_SavesFieldsWithoutChangingStatus()
    {
    ...
    [Fact]
    public async Task SaveConclusionAsync_CaseAlreadyConfirmed_ThrowsBusinessExceptionWithoutSaving()
    {
        // Arrange — P2/GB-01: ca đã khoá thì không sửa được nữa dưới bất kỳ hình thức nào,
    ...
    [Fact]
    public async Task SaveConclusionAsync_ActingDoctorIsNotResponsibleDoctor_ThrowsBusinessExceptionWithoutSaving()
    {
    ...
    [Fact]
    public async Task SaveConclusionAsync_CaseNotFound_ThrowsResourceNotFoundException()
    {
        // Arrange
        _cases.Setup(r => r.GetForUpdateAsync(It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
              .ReturnsAsync((Case?)null);
    ...
    public async Task UpdateDiagnosesAsync_CaseNotFound_ThrowsResourceNotFoundException()
    {
        ...
        // Act & Assert
        await Assert.ThrowsAsync<ResourceNotFoundException>(
            () => _sut.UpdateDiagnosesAsync(missingCaseId, Guid.NewGuid(), request, TestContext.Current.CancellationToken));
    }
